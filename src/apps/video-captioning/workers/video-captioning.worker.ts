...
self.addEventListener("message", async (event) => {
  ...
  if (type === "load") {
    ...
    try {
      ...
      const estimatedTokens = Math.max(10, audioDuration * 3.5);
      let stepCount = 0;
      ...
      const streamer = new TextStreamer(transcriber.tokenizer, {
        ...
        callback_function: () => {
          stepCount++;
          const percent = Math.min(99, Math.round((stepCount / estimatedTokens) * 100));
          self.postMessage({
            type: "generating_progress",
            statusText: `Captioning (${percent}%)...`,
            progress: percent,
          });
        },
      });
      ...
      const finalOutput = Array.isArray(result) ? result[0] : result;
      self.postMessage({ type: "complete", result: finalOutput.chunks });
    } catch (err: unknown) {
