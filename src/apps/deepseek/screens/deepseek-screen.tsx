...
const DeepseekScreen = () => {
  ...
  useEffect(() => {
    if (!worker.current) {
      worker.current = new DeepseekWorker();
      ...
      {(status === "idle" || status === "error") && messages.length === 0 && (
        <div className="flex h-full flex-col items-center justify-center space-y-6">
          <div className="bg-primary/10 mb-4 flex h-20 w-20 items-center justify-center rounded-full">
            <Deepseek className="text-primary h-10 w-10" />
          </div>
          <H1 className="text-3xl">DeepSeek R1 - 1.5B</H1>
