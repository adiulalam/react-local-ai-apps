...
describe("translategemma worker-message-handler", () => {
  const callbacks = {
    setStatus: vi.fn(),
  ...
  it("handles update message", () => {
    handler(
      new MessageEvent("message", {
        data: { type: "update", output: "Hola", tps: 12.5, numTokens: 4 },
      })
    );
    expect(callbacks.onUpdate).toHaveBeenCalledWith("Hola", 12.5, 4);
  });

  it("handles complete message", () => {
    handler(new MessageEvent("message", { data: { type: "complete", result: ["Hola"] } }));
    expect(callbacks.setStatus).toHaveBeenCalledWith("complete");
    expect(callbacks.onComplete).toHaveBeenCalledWith(["Hola"]);
  });

  it("handles error message", () => {
    handler(new MessageEvent("message", { data: { type: "error", error: "Translation failed" } }));
    expect(callbacks.setStatus).toHaveBeenCalledWith("error");
    expect(callbacks.setErrorMsg).toHaveBeenCalledWith("Translation failed");
  });
});
