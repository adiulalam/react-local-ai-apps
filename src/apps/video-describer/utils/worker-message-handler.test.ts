...
describe("video-describer worker-message-handler", () => {
  it("should handle different worker messages and call appropriate callbacks", () => {
    ...
    handler(
      new MessageEvent("message", { data: { type: "complete", result: "a person walking" } })
    );
    expect(callbacks.setStatus).toHaveBeenCalledWith("complete");
    expect(callbacks.onComplete).toHaveBeenCalledWith("a person walking");
